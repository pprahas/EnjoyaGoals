...
router.post("/update", async (req, res) => {
  ...
  try {
    ...
    switch (req.body.fieldName) {
      case "name":
        roomToUpdate.update({ name: req.body.value });
        break;
      case "password":
        roomToUpdate.update({ password: req.body.value });
        break;
      case "owner":
        roomToUpdate.update({ owner: req.body.value });
        break;
      case "type":
        roomToUpdate.update({ type: req.body.value });
        break;
      case "users":
        ...
        // "remove":	true
        if (req.body.add) {
          roomToUpdate.users.push(req.body.value);
          const user = await User.findById(req.body.value);

          roomToUpdate.voteKickMember.set(
            req.body.value,
            user.username + "!@#$" + "0"
          );
        } else if (req.body.remove) {
          roomToUpdate.users.pull(req.body.value);
        } else {
          console.log(
            "SOMETHING IS WRONG WITH THE /room/update ROUTE FOR UPDATING users"
          );
      ...
      case "assignedTasks":
        ...
        // "remove":	true
        if (req.body.add) {
          roomToUpdate.assignedTasks.push(req.body.value);
        } else if (req.body.remove) {
          roomToUpdate.assignedTasks.pull(req.body.value);
        } else {
          console.log(
            "SOMETHING IS WRONG WITH THE /room/update ROUTE FOR UPDATING assignedTasks"
          );
          return res.status(501).json({
            msg: "Sorry, we messed up and can't do this operation right now. We'll fix it soon.",
          });
      ...
      case "completedTasks":
        ...
        // "remove":	true
        if (req.body.add) {
          roomToUpdate.completedTasks.push(req.body.value);
        } else if (req.body.remove) {
          roomToUpdate.completedTasks.pull(req.body.value);
        } else {
          console.log(
            "SOMETHING IS WRONG WITH THE /room/update ROUTE FOR UPDATING completedTasks"
          );
          return res.status(501).json({
            msg: "Sorry, we messed up and can't do this operation right now. We'll fix it soon.",
          });
      ...
      case "teamTasks":
        ...
        // "remove":	true
        if (req.body.add) {
          roomToUpdate.teamTasks.push(req.body.value);
        } else if (req.body.remove) {
          roomToUpdate.teamTasks.pull(req.body.value);
        } else {
          console.log(
            "SOMETHING IS WRONG WITH THE /room/update ROUTE FOR UPDATING teamTasks"
          );
          return res.status(501).json({
            msg: "Sorry, we messed up and can't do this operation right now. We'll fix it soon.",
          });
