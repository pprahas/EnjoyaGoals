...
const Notif = (props) => {
  try {
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        if (
          notifList.length &&
          list.length &&
          autoDeleteCheck(list[autoDelIndex])
        ) {
    ...
    const autoDeleteCheck = (notifToCheck) => {
      if (notifToCheck.type === "info" || notifToCheck.type === "success") {
        return true;
      }
      ...
      <>
        <div className="notification-container bottom-right">
          {list.map((notif, i) => (
            <div
              key={i}
              className={`notification notif bottom-right ${notif.type}`}
            >
              <button onClick={() => deleteToast(notif.id)}>X</button>
              <div className="notification-image">{getIcon(notif.type)}</div>
              <div>
